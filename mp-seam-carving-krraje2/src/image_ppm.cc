...
void ImagePPM::HorizontalCarver(const int* values) {
  ...
  for (int col = 0; col < width_; col++) {
    ...
    for (int row = 0; row < height_ - 1; row++) {
      if (row != values[col]) {
        newimage[row][col] = GetPixel(orig_row, col);
      } else {
        orig_row++;
        newimage[row][col] = GetPixel(orig_row, col);
      }
...
void ImagePPM::VerticalCarver(const int* values) {
  ...
  for (int row = 0; row < height_; row++) {
    ...
    for (int col = 0; col < width_ - 1; col++) {
      if (col != values[row]) {
        newimage[row][col] = GetPixel(row, orig_col);
      } else {
        orig_col++;
        newimage[row][col] = GetPixel(row, orig_col);
      }
